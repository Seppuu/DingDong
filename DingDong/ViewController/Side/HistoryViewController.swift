...
class HistoryViewController: BaseViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func setTableView() {
        ...
        historyTableView.emptyDataSetSource = self
        historyTableView.emptyDataSetDelegate = self
        
        
        historyTableView.addInfiniteScrollingWithHandler {  [weak self] in
    ...
    func getFirstPageData() {
        ...
        getDataFromServerWith(1, pageSize: pageSize) { (success, json, error) in
            ...
            self.historyTableView.pullToRefreshView?.stopAnimating()
        }
    ...
    func footerRefresh() {
        ...
        if currentPageNumber <= totalPageNumber {
            ...
            delay(1.0, closure: {  [weak self] in
                ...
                if let strongSelf = self {
                    strongSelf.historyTableView.infiniteScrollingView?.stopAnimating()
    ...
    func loadMoreDatawith(_ pageNumber:Int,pageSize:Int) {
        ...
        getDataFromServerWith(pageNumber, pageSize: pageSize) { (success, json, error) in
            ...
            self.historyTableView.infiniteScrollingView?.stopAnimating()
        }
    }
    ...
    fileprivate func getDataFromServerWith(_ pageNumer:Int,pageSize:Int,completion:@escaping (_ success:Bool,_ json:JSON?,_ error:String?) -> Void) {
        ...
        NetworkManager.sharedManager.getHistoryLessonListWith(pageNumer, pageSize: pageSize, completion: completion)
    }
    ...
    func insertMoreDataToViewWith(_ originalCount:Int ,count:Int) {
        ...
        historyTableView.insertRows(at: listOfIndexpath, with: .fade)
